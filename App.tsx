    ...
    <div className='space-y-4'>
      ...
      {items.length === 0 ? (
        <p className='text-center text-slate-500 py-8'>
          Bu ünite için kelime listesi bulunmamaktadır.
        </p>
      ) : (
...
// 2. Grammar View
const GrammarView: React.FC<{ rules: UnitContent['grammar'] }> = ({
  rules,
}) => {
  return (
    <div className='space-y-6'>
      ...
      {rules.length === 0 ? (
        <p className='text-center text-slate-500 py-8'>
          Bu ünite için dil bilgisi kuralı bulunmamaktadır.
        </p>
      ) : (
        rules.map((rule, idx) => (
          <div
          ...
          >
            ...
            <div className='bg-slate-50 p-4 rounded-lg'>
              <h5 className='font-semibold text-slate-600 text-sm mb-2'>
                Örnekler / أمثلة
              </h5>
              <ul className='space-y-2'>
                {rule.examples.map((ex, i) => (
                  <li
                    key={i}
                    className='flex flex-col sm:flex-row sm:items-center sm:justify-between border-b border-slate-200 pb-2 last:border-0 last:pb-0'
                  >
...
// 3. Conversation View
const ConversationView: React.FC<{ dialogues: UnitContent['dialogue'] }> = ({
  dialogues,
}) => {
  return (
    <div className='space-y-8 max-w-3xl mx-auto'>
      ...
      {dialogues.length === 0 ? (
        <p className='text-center text-slate-500 py-8'>
          Bu ünite için konuşma bulunmamaktadır.
        </p>
      ) : (
        dialogues.map((dialogue, dIdx) => (
          ...
          >
            ...
            <div className='mb-8 bg-slate-100 rounded-xl p-6 text-center border-2 border-dashed border-slate-300 relative overflow-hidden group'>
              ...
              <p className='font-semibold text-slate-700 max-w-lg mx-auto leading-relaxed'>
                {dialogue.imageDescription}
              </p>
              <p className='text-xs text-teal-600 mt-2 font-medium uppercase tracking-wide'>
                Görsel Bağlam / سياق بصري
              </p>
            </div>
                ...
                return (
                  ...
                  >
                    <div
                      ...
                      ${
                        isEven
                          ? 'bg-white border-l-4 border-teal-500 rounded-tl-none'
                          : 'bg-teal-50 border-r-4 border-teal-600 rounded-tr-none'
                      }
                    `}
                    >
                      <div className='flex items-center gap-2 mb-1'>
                        <span className='text-xs font-bold uppercase tracking-wider opacity-60 text-slate-600'>
                          {line.speaker}
                        </span>
...
// 4. Reading View
const ReadingView: React.FC<{ readings: UnitContent['reading'] }> = ({
  readings,
}) => {
  return (
    <div className='space-y-8 max-w-3xl mx-auto'>
      ...
      {readings.length === 0 ? (
        <p className='text-center text-slate-500 py-8'>
          Bu ünite için okuma parçası bulunmamaktadır.
        </p>
      ) : (
          ...
          >
            ...
            <div className='bg-slate-100 p-8 text-center border-b border-slate-200'>
              ...
              <p className='text-slate-600 font-medium italic'>
                {reading.imageDescription}
              </p>
            </div>

            <div className='p-6 sm:p-8'>
              <h4 className='text-2xl font-bold text-teal-800 mb-6 text-center'>
                {reading.title}
              </h4>

              <div className='prose prose-lg text-slate-700 leading-loose mb-6'>
                <div className='flex justify-end mb-2'>
                  <AudioButton text={reading.contentTr} />
                </div>
...
// 5. Game View
const GameView: React.FC<{ vocabulary: VocabularyItem[] }> = ({
  vocabulary,
}) => {
  ...
  const trItems = useMemo(
    () => [...matchingSubset].sort(() => Math.random() - 0.5),
    [matchingSubset]
  );
  ...
  const handleCardClick = (type: 'tr' | 'ar', val: string) => {
    if (selectedTr) {
      ...
      // Check match
      const correctMatch =
        matchingSubset.find((v) => v.turkish === selectedTr)?.arabic === val;
      if (correctMatch) {
    ...
    return (
      <div className='space-y-6 text-center max-w-lg mx-auto mt-10'>
        <h3 className='text-xl font-bold text-slate-800 mb-6'>
          Oyun Seçiniz / اختر لعبة
        </h3>
        <div className='grid grid-cols-2 gap-4'>
          <button
            onClick={() => setGameMode('match')}
            className='p-8 bg-white rounded-2xl shadow-sm border border-slate-200 hover:border-teal-500 hover:shadow-md transition-all flex flex-col items-center gap-4 group'
          >
            ...
            <span className='font-bold text-slate-700'>
              Eşleştirme <br />
              <span className='text-xs font-normal arabic-text text-slate-500'>
                مطابقة
              </span>
            </span>
          </button>
      ...
      <div className='max-w-md mx-auto text-center'>
        <div className='flex justify-between items-center mb-6'>
          <h3 className='text-xl font-bold text-slate-800'>
            Kelime Kartları ({currentCardIndex + 1}/{vocabulary.length})
          </h3>
          <button
        ...
        >
          ...
          >
            ...
            <div className='absolute w-full h-full backface-hidden bg-white rounded-2xl shadow-md border border-slate-200 flex flex-col items-center justify-center p-8'>
              <span className='text-3xl font-bold text-slate-800 mb-4'>
                {currentItem.turkish}
              </span>
            ...
            <div className='absolute w-full h-full backface-hidden bg-teal-600 text-white rounded-2xl shadow-md border border-teal-600 flex flex-col items-center justify-center p-8 rotate-y-180'>
              <span className='text-3xl font-bold arabic-text mb-4'>
                {currentItem.arabic}
              </span>
            </div>
          </div>
        ...
        <div className='mt-8 flex justify-center gap-4'>
          <button
            ...
            className='px-6 py-3 bg-white border border-slate-200 text-slate-700 rounded-xl hover:bg-slate-50 shadow-sm'
          >
            Önceki
          </button>
          <button
            onClick={handleNextCard}
            className='px-6 py-3 bg-teal-600 text-white rounded-xl hover:bg-teal-700 shadow-md'
          >
            Sonraki
          </button>
        </div>
  ...
  return (
    <div>
      ...
      {feedback && (
        <div
          className={`fixed bottom-10 left-1/2 transform -translate-x-1/2 px-8 py-4 rounded-full text-white font-bold shadow-lg z-50 text-lg ${
...
const QuizView: React.FC<{ unit: UnitContent }> = ({ unit }) => {
  ...
  const generateQuestions = React.useCallback(() => {
    ...
    const vocabQuestions: QuizQuestion[] = unit.vocabulary.map((vocabItem) => {
      ...
      const options = [...distractors, vocabItem.arabic].sort(
        () => Math.random() - 0.5
      );
  ...
  if (generatedQuestions.length === 0)
    return <div className='text-center p-8'>Soru bulunamadı...</div>;
    ...
    return (
      <div className='text-center py-12 bg-white rounded-2xl shadow-sm border border-slate-100 max-w-xl mx-auto'>
        ...
        <p className='text-lg text-slate-600 mb-6'>
          Skorunuz: {score} / {generatedQuestions.length}
        </p>
    ...
    <div className='max-w-xl mx-auto'>
      {unit.id === 9 && (
        <div className='mb-4 flex justify-end'>
          <button
            onClick={handleRefresh}
            className='text-sm text-teal-600 flex items-center gap-1 hover:underline bg-teal-50 px-3 py-1 rounded-lg'
          >
      ...
      <div className='flex justify-between items-center mb-6'>
        <span className='text-sm font-bold text-teal-600 bg-teal-50 px-3 py-1 rounded-full'>
          Soru {currentQ + 1}/{generatedQuestions.length}
        </span>
        ...
        <div className='space-y-3'>
          {question.options.map((opt, idx) => {
            ...
            if (isAnswered) {
              ...
              btnClass +=
                'border-slate-100 hover:border-teal-300 hover:bg-teal-50 text-slate-700';
            }
      ...
      {isAnswered && (
        ...
        >
          {currentQ === generatedQuestions.length - 1
            ? 'Sonucu Gör'
            : 'Sonraki Soru'}
  ...
  const renderModule = () => {
    ...
    switch (activeModule) {
      case ModuleType.VOCABULARY:
        return <VocabularyView items={activeUnit.vocabulary} />;
      case ModuleType.GRAMMAR:
        return <GrammarView rules={activeUnit.grammar} />;
      case ModuleType.CONVERSATION:
        return <ConversationView dialogues={activeUnit.dialogue} />;
      case ModuleType.READING:
        return <ReadingView readings={activeUnit.reading || []} />;
      case ModuleType.GAME:
        return <GameView vocabulary={activeUnit.vocabulary} />;
      case ModuleType.QUIZ:
        return <QuizView unit={activeUnit} />;
      default:
        return null;
    }
  };
    ...
    <div className='min-h-screen bg-slate-50 flex flex-col font-sans'>
      ...
      <header className='bg-white border-b border-slate-200 sticky top-0 z-10 shadow-sm'>
        <div className='max-w-5xl mx-auto px-4 h-16 flex items-center justify-between'>
          ...
          >
            <div className='w-9 h-9 bg-teal-600 rounded-xl flex items-center justify-center text-white font-bold shadow-sm group-hover:scale-105 transition-transform'>
              A
            </div>
            <span className='font-bold text-slate-800 tracking-tight hidden sm:inline group-hover:text-teal-700 transition-colors'>
              Al-Furqan Türkçe
            </span>
          </div>
          ...
          <div className='flex items-center gap-3'>
            <div className='text-right'>
              <h1 className='text-sm font-bold text-slate-900 arabic-text'>
                جمعية الفرقان
              </h1>
          ...
          <div className='animate-fade-in space-y-12'>
            <div className='text-center py-8'>
              <h2 className='text-3xl sm:text-4xl font-bold text-slate-900 mb-4'>
                Ders Kitabı Seçiniz <span className='text-teal-600'>/</span>{' '}
                <span className='arabic-text'>اختر الكتاب الدراسي</span>
              </h2>
              <p className='text-slate-600'>
                Lütfen çalışmak istediğiniz kitabı seçin.
              </p>
            </div>
            ...
            <div className='grid md:grid-cols-2 gap-8 max-w-4xl mx-auto'>
              {COURSES.map((course) => (
                ...
                >
                  <div
                    className={`absolute inset-0 opacity-10 group-hover:opacity-15 transition-opacity ${course.coverColor}`}
                  ></div>
                  <div className={`absolute top-0 right-0 p-6 opacity-20`}>
                    <Library
                      size={120}
                      className={
                        course.id === 'istanbul_a1'
                          ? 'text-red-900'
                          : 'text-blue-900'
                      }
                    />
                  ...
                  <div className='relative z-10'>
                    <span
                      className={`inline-block px-3 py-1 rounded-full text-xs font-bold text-white mb-3 ${course.coverColor}`}
                    >
                      A1 SEVİYE
                    </span>
                    ...
                    <h4 className='text-xl font-medium text-slate-600 arabic-text'>
                      {course.titleAr}
                    </h4>
                  </div>
                </button>
            ...
            <div className='grid md:grid-cols-2 lg:grid-cols-3 gap-6'>
              {activeCourse?.units.map((unit) => (
                <button
                  ...
                  className={`group bg-white rounded-2xl p-6 shadow-sm border border-slate-200 hover:border-teal-500 hover:shadow-lg transition-all text-left relative overflow-hidden ${
                    unit.id === 9 ? 'ring-2 ring-teal-500 ring-offset-2' : ''
                  }`}
                >
                  <div
                    className={`absolute top-0 right-0 w-32 h-32 rounded-bl-full -mr-8 -mt-8 transition-colors ${
                      unit.id === 9
                        ? 'bg-teal-100'
                  ...
                  ></div>
                  <span
                    className={`inline-block px-3 py-1 text-xs font-bold rounded-full mb-4 relative z-10 ${
                      unit.id === 9
                  ...
                  <h3 className='text-xl font-bold text-slate-800 mb-1 group-hover:text-teal-700 transition-colors relative z-10'>
                    {unit.titleTr}
                  </h3>
                  <h4 className='text-lg font-medium text-slate-500 arabic-text mb-4 relative z-10'>
                    {unit.titleAr}
                  </h4>
                  <p className='text-sm text-slate-600 relative z-10 line-clamp-2'>
                    {unit.description}
                  </p>
                </button>
              ))}
          ...
          <div className='animate-fade-in'>
            ...
            <div className='flex flex-wrap items-center gap-2 mb-8 bg-white p-3 rounded-full shadow-sm border border-slate-100 w-fit'>
              ...
              {activeModule && (
                <>
                  <ChevronRight size={16} className='text-slate-300' />
                  <span className='text-teal-600 font-bold px-2 capitalize bg-teal-50 rounded-full py-1 text-xs sm:text-sm'>
                    {activeModule}
                  </span>
                </>
              )}
            ...
            {!activeModule ? (
              <div>
                <div className='text-center mb-10'>
                  ...
                  <h3 className='text-2xl text-slate-600 arabic-text'>
                    {activeUnit?.titleAr}
                  </h3>
                </div>
                <div className='grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4'>
                  {[
                    {
                      id: ModuleType.VOCABULARY,
                      icon: BookOpen,
                      label: 'Kelime',
                      labelAr: 'المفردات',
                    },
                    {
                      id: ModuleType.GRAMMAR,
                      icon: Puzzle,
                      label: 'Dil Bilgisi',
                      labelAr: 'القواعد',
                    },
                    {
                      id: ModuleType.CONVERSATION,
                      icon: MessageCircle,
                      label: 'Konuşma',
                      labelAr: 'المحادثات',
                    },
                    ...
                    >
                      ...
                      <div className='text-center'>
                        <div className='font-bold text-slate-800 text-sm group-hover:text-teal-700'>
                          {m.label}
                        </div>
                        <div className='text-xs text-slate-500 arabic-text mt-1 group-hover:text-teal-600'>
                          {m.labelAr}
                        </div>
                      </div>
                    </button>
      ...
      <footer className='bg-white border-t border-slate-200 mt-12 py-8'>
        <div className='max-w-5xl mx-auto px-4 text-center'>
          <p className='text-slate-500 text-sm mb-2'>
            © 2025 Al-Furkan Türkçe Öğrenim Platformu
          </p>
          <p className='text-teal-700 font-bold arabic-text text-lg'>
            مع تحيات جمعية الفرقان
          </p>
        </div>
      </footer>
    </div>
